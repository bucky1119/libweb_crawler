...
class Website18Crawler(AbstractCrawler):
    ...
    def parse_article_content(self, container_selector: Selector) -> ArticleContent:
        ...
        :return:
        """
        article_date_str = container_selector.xpath('.//dd[@class="date"]/text()').get()
        # 构造 ArticleContent 对象，其他字段可根据需求补充
        article = ArticleContent(
            title=container_selector.xpath('.//a/text()').get().replace('\t', '').replace('\n', ''),
            author='',
            ...
            nation="韩国",# 无
            post_agency="韩国农业部门",# 
            article_date = datetime.strptime(article_date_str.strip(), '%Y.%m.%d'),  # 调整日期格式
