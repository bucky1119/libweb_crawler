...
class Website8Crawler(AbstractCrawler):
    ...
    async def crawl(self) -> List[ArticleContent]:
        ...
        while self.running_state and self.page_count <= self.PAGE_SIZE:
            ...
            container_list = selector.xpath('//article')
            ...
            self.url = urljoin(self.url, selector.xpath('//a[@aria-label="Next"]/@href').get())
    ...
    def parse_article_content(self, container_selector: Selector) -> ArticleContent:
        ...
        :return:
        """
        article_date_str = container_selector.xpath('.//div[@class="tile-cat"]/div[@class="date"]/text()').get()
        # 构造 ArticleContent 对象，其他字段可根据需求补充
        article = ArticleContent(
            title=container_selector.xpath('.//div[@class="tile-content"]//h3/text()').get(),
            author='',
            #link_url=container_selector.xpath('.//h2/a/@href').get(),
            link_url=urljoin(self.url, container_selector.xpath('.//a[@class="tile-link"]/@href').get()),
            text=container_selector.xpath('.//span[@class="tile-teaser"]/text()').get(),
            nation="丹麦",# 无
            post_agency="哥本哈根大学",# 
            article_date = datetime.strptime(article_date_str.strip(), '%Y.%m.%d'),  # 使用当前日期，可根据需求调整
