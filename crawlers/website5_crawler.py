...
class Website5Crawler(AbstractCrawler):
    ...
    async def crawl(self) -> List[ArticleContent]:
        ...
        while self.running_state and self.page_count <= self.PAGE_SIZE:
            ...
            self.url = selector.xpath('//nav[@class="pagination"]/a[last()-1]/@href').get()
            #self.url = urljoin(self.url, selector.xpath('//nav[@class="pagination"]/a[last()-1]/@href').get())
    ...
    def parse_article_content(self, container_selector: Selector) -> ArticleContent:
        ...
        :return:
        """
        article_date_str = container_selector.xpath('.//div[2]/header/span/time/text()').get()
        # 构造 ArticleContent 对象，其他字段可根据需求补充
        article = ArticleContent(
            title=container_selector.xpath('.//h2/a/text()').get(),
            author=container_selector.xpath('//a[@rel="author"]/text()').get(),
            link_url=container_selector.xpath('.//h2/a/@href').get(),
            text=container_selector.xpath('.//p/text()').get(),
            nation="澳大利亚",# 无
            post_agency="西澳大学",# 
            article_date = datetime.strptime(article_date_str.strip(), '%B %d, %Y'),  # 使用当前日期，可根据需求调整
            # 以下默认字段，暂时不需要调整
            info_type="T0",
            domain="F0",
            subject="AG0",
