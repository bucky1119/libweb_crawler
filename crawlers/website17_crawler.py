...
class Website17Crawler(AbstractCrawler):
    ...
    def parse_article_content(self, container_selector: Selector) -> ArticleContent:
        ...
        :return:
        """
        article_date_str = container_selector.xpath('.//p[@class="Teaser-date"]/text()').get()
        ...
        article = ArticleContent(
            ...
            text=container_selector.xpath('.//p[@class="Teaser-intro"]/text()').get(),
            nation="法国",# 无
            post_agency="法国农业食品环境研究院",# 
            article_date = datetime.strptime(article_date_str.strip(), '%d %B %Y'),  # 调整日期格式
